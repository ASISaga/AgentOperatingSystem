#!/usr/bin/env python3
"""
Direct test for YAML agent configuration loading.
Imports agents directly without going through package __init__.py
"""
...
# Import directly from agent modules
import AgentOperatingSystem.agents.perpetual
import AgentOperatingSystem.agents.purpose_driven
import AgentOperatingSystem.agents.leadership_agent
import AgentOperatingSystem.agents.cmo_agent
...
def test_yaml_loading():
    ...
    try:
        ...
        if Path(yaml_path).exists():
            ...
            print(f"  - MCP Tools: {len(agent.mcp_tools_config)} tools configured")
        else:
    ...
    try:
        ...
        # Test invalid purpose type for CMO
        try:
            yaml_path = "config/agents/cmo_agent.yaml"
            if Path(yaml_path).exists():
                agent = CMOAgent.from_yaml(yaml_path)
                try:
                    ...
                    print(f"✗ Should have raised ValueError for invalid purpose")
                    return False
                except ValueError:
                    print(f"✓ Correctly raised ValueError for invalid purpose type")
        except Exception as e:
            print(f"⚠ Skipping invalid purpose test: {e}")
